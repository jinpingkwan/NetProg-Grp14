...
def _validate_network(value: str) -> str:
    try:
        ...
        raise typer.BadParameter(f"'{value}' is not a valid CIDR prefix (e.g. 10.0.0.0/24)")
    return value
...
def create_user(
    ...
    password: Annotated[
        str,
        typer.Option(prompt=True, hide_input=True, confirmation_prompt=True, help="Password for the new user"),
    ] = "",
) -> None:
    """Create a user account on a network device."""
    run_playbook("configure_user", host, {"new_username": username, "new_password": password})


@app.command("banner")
def set_banner(
    host: Annotated[str, typer.Argument(help="Inventory hostname of the target device")],
    message: Annotated[str, typer.Option(help="Banner text to display at login")],
) -> None:
    """Set a login banner message on a network device."""
    run_playbook("configure_banner", host, {"banner_message": message})


@app.command("interface")
def set_interface(
    host: Annotated[str, typer.Argument(help="Inventory hostname of the target device")],
    iface: Annotated[str, typer.Option(help="Interface name (e.g. Gi0/1)")],
    desc: Annotated[str, typer.Option(help="Description to set on the interface")],
) -> None:
    """Set a description on a network device interface."""
    run_playbook("configure_interface", host, {"interface_name": iface, "interface_desc": desc})


@app.command("route")
def add_route(
    host: Annotated[str, typer.Argument(help="Inventory hostname of the target device")],
    dest: Annotated[str, typer.Option(help="Destination network in CIDR notation (e.g. 10.0.0.0/24)", callback=_validate_network)],
    via: Annotated[str, typer.Option(help="Next-hop gateway IPv4 address", callback=_validate_ipv4)],
) -> None:
    """Add a static route on a network device."""
    run_playbook("configure_route", host, {"route_dest": dest, "route_gateway": via})
